...
1. Deep-read PyLESA, pycity_scheduling, cashocs, and PyDDRBG to extract direct quotes/figures we can reuse as benchmarks for FHOPS KPIs.
2. Expand this log with page/section references once we annotate the PDFs (e.g., highlight architecture diagrams, validation tables).
3. Feed the extracted criteria back into `notes/submission_readiness_dashboard.md` as soon as we verify FHOPS coverage.

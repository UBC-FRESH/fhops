...
- [ ] Asset-generation plan:
  ...
  - [ ] **Synthetic scaling sweep:** Script (`scripts/run_synthetic_sweep.py`) that generates synthetic small/medium/large tiers (fixed RNG seeds), runs a single SA pass per tier, and plots runtime vs. scenario size (PDF) akin to GROMACS scaling charts---comment on .

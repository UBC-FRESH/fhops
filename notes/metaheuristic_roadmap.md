...
- 2024-12-01 snapshot (`tmp/bench_gap`): tiny7 MILP 4 088.22 vs. greedy seed/SA/ILS/Tabu 4 093.24 (same production, slightly lower mobilisation). small21 MILP 8 797.07 vs. greedy seed/SA 8 719.05 (≈0.9 % gap with identical production). On these ladders the greedy initializer already lands in near-optimal territory; the metaheuristics simply preserve it. Future tuning work needs scenarios where the seed is weaker (med42/large84) to demonstrate operator value.
- Follow-up: once med42/large84 reshape is complete, revisit the MILP objective weights to include a modest makespan penalty so the MILP doesn’t prefer stretched schedules when cheaper mobilisation/shorter completion plans exist (e.g., small21 greedy vs. MILP). Track that change alongside the heuristic scoring update so both solvers stay aligned.

...
- [ ] **Roadside processors / loaders / hoe chuckers (Labelle et al. 2016/2018; FPDat loader datasets)**
  ...
  - [x] Provide helper functions for `roadside_processor` and `loader`, exposing key predictors (piece size, log sort count, decking distance) and regression tests. Tie processor cost factors back to `notes/reference/Development_and_Validation_of_.pdf` (OpCost) and `notes/reference/harvestcost-woodstock.pdf` so CLI outputs match Woodstock/OpCost conventions.
    - [x] Processor helper interface now supports `--processor-model berry2019` (piece size, tree form, crew, utilisation) and `--processor-model labelle2019_dbh` (DBH input, species = spruce/beech, treatment = clear_cut/selective_cut) backed by `estimate_processor_productivity_berry2019(...)` and `estimate_processor_productivity_labelle2019_dbh(...)`. Docs/CLI explicitly warn that the Labelle regressions are PMH₀ outputs for hardwood-dominated Bavarian blocks (rare in BC, handy for export scenarios).
  ...
  - [ ] Digitise the Berry/Labelle landing-processor datasets (`notes/reference/Berry, Nick_Final-Dissertation.pdf`, `administrator,+jfe5_2tp04.pdf`) into structured tables (CSV/JSON) with clear predictor definitions so helpers/tests can reuse the published coefficients).
    ...
    - [x] Labelle et al. productivity models staged via Arnvik (2024) Appendices 8–10 (see `data/productivity/processor_labelle_models.json`); DBH-based variants now power the hardwood helper. Next step: expose the volume-based (`V₆/V₆d`) variants + earlier Labelle studies once predictor definitions are mapped.

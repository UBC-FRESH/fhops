...
- [ ] **Skyline yarders / helicopter longline (Aubuchon 1982; Böhm & Kanzian 2023; Arnvik helicopter refs)**
  ...
  - ### Skyline action plan (refresh)
    ...
    - [ ] **Skyline data shopping list (FPInnovations Nov 2025 drop)** – new PDFs staged under `notes/reference/fpinnovations/` map to these outstanding needs:
        ...
        - **TN258 intermediate-support telemetry** – new dataset fields capture the tree-selection rules (≤6 m spacing, 50–60 cm DBH, rigging 12–22 m) and the recorded skyline/guyline tension ranges (empty 88–92 kN, suspended 109–140 kN, hang-ups 147 kN; guyline spikes 24–37 kN). TN258 does *not* publish intermediate-support move/setup minutes or Cat D8/Timberjack utilisation, so we derived proxies from TN-157: Cat D8 backspar standby = 0.25 SMH/SMH (mean road-change ÷ yarding), Timberjack 450 trail support = 0.14 SMH/SMH (lower quartile). These ratios now live in `fncy12_tmy45_mini_mak.json` and `grapple_yarder_tmy45` absorbs the CPI-adjusted surcharges using the TR-45 bulldozer/skidder rates. TN258 tension envelopes are now wired into `estimate-skyline-productivity` (TR125 multi-span + the new `fncy12-tmy45` model) with automatic warnings + telemetry flags whenever lateral pulls exceed 30 m, so analysts and costing both see when they’ve left the safe envelope; replace the standby proxies with authentic 1992 payroll/fuel logs if they resurface.
        - **Small-span skyline regressions** – TN54 (Micro Master yarder), FNG73 (Hi‑Skid short-yarding truck), TN39 (log-sorting logistics), and TN173 (Eastern compact yarders) supply the payload/distance/timing tables needed to round out our micro-yarder presets. We still need regression-ready JSON plus machine-rate hooks for each system.
          - *Progress 2025‑11‑30:* TN54 is fully wired (CLI preset + JSON), TN39’s footprint/cost curves live in `notes/reference/skyline_small_span_notes.md`, TN173 now delivers end-to-end coverage (structured JSON + CLI presets + machine-rate roles), the FNG73 Hi-Skid truck ships as both a dataset (`data/reference/fpinnovations/fng73_hi_skid.json`) and a CLI skyline model (`--model hi-skid` with optional travel-inclusive reporting), the default harvest-system registry now includes `cable_micro_*` / `cable_micro_hi_skid`, `data/machine_rates.json` carries the derived `skyline_hi_skid` entry, and the Thunderbird TMY45 + Mini-Mak II operation from FNCY12/TN258 is now summarized in `data/reference/fpinnovations/fncy12_tmy45_mini_mak.json` (productivity, rigging geometry, tension notes). Synthetic tiers now sample every `cable_micro_*` preset plus Hi-Skid; still pending: validating the Hi-Skid cost entry against Alfa Fab/FPInnovations invoices and mining TN258 for intermediate-support timing/D8 standby data so the new `grapple_yarder_tmy45` role can absorb authentic payroll/fuel splits.

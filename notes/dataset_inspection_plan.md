...
- [ ] **Skyline yarders / helicopter longline (Aubuchon 1982; Böhm & Kanzian 2023; Arnvik helicopter refs)**
  ...
  - ### Skyline action plan (refresh)
    ...
    - [x] **Skyline costing defaults** – Build CPI-aware machine-rate entries for representative skyline yarders (Madill 009, Cypress 7280C, Skylead C40) plus rigging/crew assumptions, and thread them into `inspect-machine` / `--show-costs` / harvest-system overrides.
      - [x] **Next action (Item 1)** – Madill 009 hooked up: TN147 owning/operating splits now live in `data/machine_rates.json` (`grapple_yarder_madill009` + updated `tower_yarder`). The CLI cost banner switches to this role whenever `--grapple-yarder-model tn147` (or related harvest-system presets) is selected, so skyline outputs finally cite the BC highlead rental rate.
      - [x] **Next action (Item 2)** – Skylead C40/TMY45 costs extracted from TN-201 (Vernon strip-cut case): `data/machine_rates.json` now ships the `grapple_yarder_skyleadc40` entry (18.85 $/SMH owning + 125.99 $/SMH operating, 1991 CAD) so FNCY12/TR125-style intermediate-support corridors can quote the proper CPI-adjusted skyline rate ahead of the harvest-system overrides.
      - [x] **Next action (Item 3)** – Cypress 7280B/UH14 backspar costs now live in `data/machine_rates.json` as `grapple_yarder_cypress7280`; the CLI maps the TN157 helper/`cable_running*` presets to this role so skyline costing finally references the published $259/h + $31/h split (1991 CAD + CPI).
      - [x] **Next action (Item 4)** – ADV5N28 Madill 071 (Acme Pow’-R Block) skyline costs wired into `grapple_yarder_adv5n28`; both ADV5N28 presets and the `cable_running_adv5n28_*` harvest systems now reference the Advantage Vol. 5 No. 28 owning/operating split instead of inheriting Cypress defaults.
      - [x] **Next action (Item 5)** – Reconstructed the Thunderbird TMY45 + Mini-Mak (FNCY12/TN258) machine-rate entry: converted LeDoux (1984) TMY-45 hourly charges from USD 1984 → CAD 1992 via the StatCan CPI/FX trail, scaled the labour column to the FNCY12 5.5-person rigging crew, and amortised the CAD 33.5 k Mini-Mak II + skyline support jacks over five years (1 200 SMH/year). The resulting `grapple_yarder_tmy45` owning/operating split now lives in `data/machine_rates.json`, `inspect-machine --machine-role grapple_yarder_tmy45` reports the CPI-adjusted rate, and `--show-costs` finally references a BC-specific Thunderbird value instead of the Skylead surrogate. Follow-ups: (1) capture authentic 1992 BC payroll/fuel/repair ledgers to validate or replace the reconstructed labour/fuel portions, (2) add the Cat D8 + Timberjack 450 standby costs once we mine TN258 intermediate-support timing, and (3) expose the new skyline/guyline tension envelopes in the helper/telemetry so analysts get warnings when lateral pulls exceed the 30 m ceiling that triggered the TN258 hang-ups.
    - [x] **Manual falling (TN-98 integration)** – `fhops.dataset estimate-skyline-productivity` now has `--manual-falling`, `--manual-falling-species`, and `--manual-falling-dbh-cm` flags that surface the TN-98 cutting/limbing/fixed minutes + $/tree/$·m⁻³ alongside the skyline output. Harvest systems that specify `hand_faller`/`hand_or_mech_faller` jobs (e.g., `cable_micro_*`, `cable_running*`, `cable_standing`, `helicopter`) now carry manual-falling overrides in their `SystemJob.productivity_overrides`, so `--harvest-system-id ...` automatically prints the right species/DBH defaults (hemlock 32.5 cm for micro systems, Douglas-fir 52.5 cm for long-span running skylines, etc.). Telemetry records the manual falling assumption so costing can audit it later. Next follow-up: decide if we feed the TN-98 $/m³ straight into `--show-costs` or keep it informational until skyline + falling costs are combined in a single banner.
      - [ ] **LeDoux (1984) residue yarding regressions (USDA NE RP)** – `notes/reference/ne_1984_ledoux_002.pdf` digitizes regression equations + hourly costs for four skyline systems (Skagit BU-94 shotgun/highlead, Washington 208E, Thunderbird TMY-45). Plan:
        - [x] Transcribe Table 3 regression formulas (converted ft/ft³ inputs to metric-friendly CLI flags) so the `ledoux-*` skyline models can compute cycle minutes + productivity directly.
        - [x] Pull the Table 4 hourly costs (1984 USD, new-equipment assumption) into `data/machine_rates.json` as interim machine-rate entries (annotated as US residue trials so CPI/FX context is explicit).
        - [x] Capture the current BC wage/benefit references in `notes/reference/bc_iwa_wage_tables.md` (1994 TN-261 table, 1996/97 TR-119/TR-125 summaries, 1998 interior rates, 2002/2004 Advantage wage notes) so future CPI conversions cite concrete sources.
        - [x] Document outstanding wishes:
            1. **Residue utilisation penalties** – `notes/reference/skyline_residue_penalties.md` now summarises the LeDoux residue coefficients (Skagit/Washington/TMY‑45) and the TR119/TR125 partial-cut productivity deltas, and the `estimate-skyline-productivity --model ledoux-*` CLI now prints a residue-heavy warning whenever the residue delay component exceeds the merchantable component.
            2. **CPI/FX audit trail** – `notes/reference/cpi_fx_trail.md` captures the StatCan Table 18-10-0005-01 CPI values, the 2024 multipliers, and the CAD/USD averages (StatCan vector v37426 for 1979‑2004 + BoC Valet FXAUSDCAD for 2024) so every skyline cost conversion can cite a reproducible source.
    ...
    - [x] **Skyline wiring plan (no new refs)** – finish closing the loop with the datasets already in hand:
        - **Preset plumbing** – ensure every digitized skyline/helicopter dataset has a matching CLI helper + harvest-system override (TR125/TR127/Aubuchon/Kellogg/Kramer, TN147/TN157/TR122, ADV5N28, TN173, Hi-Skid, FNCY12 steady-state). Add range checks + telemetry flags wherever inputs exceed the published envelopes.
            - [x] Aubuchon/Kramer/Kellogg presets now print their calibration ranges (Wyssen 3.5–6 logs/turn, Kramer chord slope −22 % to +3 %, Kellogg lead angle ±90°/1–2 chokers) and log telemetry warnings whenever user inputs fall outside those envelopes.
            - [x] Added harvest systems for the TR125 single/multi-span and TR127 block (partial-cut) presets so skyline/lateral defaults, TR119 treatments, and TN-258 warnings prefill automatically for dataset blocks (`cable_standing_tr125_*`, `cable_partial_tr127_*`). Synthetic tiers and CLI tests cover the new overrides.
            - [x] Mirror the same treatment for the grapple presets (TN147, TN157, TR122, ADV5N28, SR54/TR75) so harvest systems/synthetic tiers can auto-populate turn volume, yarding distance, stems/turn, and CPI-aware cost roles. `default_system_registry()` now exposes dedicated IDs (`cable_highlead_tn147`, `cable_running_tn157_combined`, `cable_running_tr122_*`, `cable_running_sr54`, `cable_running_tr75_*`, plus the existing ADV5N28/FNCY12 entries). `TIER_DEFAULTS` was refreshed so medium/large synthetic tiers actually sample the new IDs, `docs/reference/harvest_systems.rst` documents the presets in a grapple-specific table (with cost-role pointers), and the CLI tests cover each harvest-system override to keep the plumbing pinned.
        - **Cost + support surfacing** – verify `inspect-machine`/`--show-costs` auto-select the CPI-aware roles (Madill 009, Cypress 7280, Skylead C40, ADV5N28, Thunderbird TMY45). Keep the TN157-derived support ratios for TMY45 but log them (and the TN258 lateral warning) every time the preset runs so downstream costing knows a proxy is in play.
        - **Docs/tests** – expand the skyline section of `docs/reference/harvest_systems.rst` with a matrix mapping source → model → cost role, call out which presets are BC vs. proxy, and add CLI/tests covering the new fncy12 variant/warning plus any remaining TR125/TR127/Aubuchon branches. ✅ Skyline cost matrix now ships in the docs (with a doc test verifying the machine-rate links), the TR-127 block table now lists the predictor/range requirements so analysts don’t have to open the JSON, and the CLI/telemetry work from the prior step keeps the warnings aligned.
    - [ ] **Skyline data shopping list (FPInnovations Nov 2025 drop)** – new PDFs staged under `notes/reference/fpinnovations/` map to these outstanding needs:
        - **TMY‑45 costing (FNCY12/TN258 addenda)** – owning/operating split reconstructed (see `grapple_yarder_tmy45`), and the TN258 skyline/guyline tension tables now live in `data/reference/fpinnovations/fncy12_tmy45_mini_mak.json`. Still need the TN258 intermediate-support timing sheets and any Jay-Vee Yarding payroll/fuel ledgers so the Cat D8 + Timberjack 450 standby costs (and true IWA wage references) can replace the current LeDoux-scaled assumptions. The new `fncy12-tmy45` skyline preset (steady-state default, optional `--fncy12-variant` for overall/steady-state-no-fire) now feeds those costs directly into telemetry/`--show-costs`, so Thunderbird corridors finally cite their own CPI-aware rate.
        - [x] **TN258 intermediate-support telemetry** – dataset now also records a ``support_monthly_summary`` derived from Table 2 (July = learning/no supports, August–October = supports installed). Each month lists productive shifts, yarder SMH, and the inferred Cat D8/Timberjack SMH (0.3335/0.2415 SMH per yarder SMH allocated by each support month’s share of support SMH), plus bucket/totals so analysts can reproduce the “supports vs. no supports” split even without stopwatch logs. TN258 still lacks direct timing, but the structured month table replaces the old prose-only assumption with a machine-readable audit trail.
        - **Small-span skyline regressions** – TN54 (Micro Master yarder), FNG73 (Hi‑Skid short-yarding truck), TN39 (log-sorting logistics), and TN173 (Eastern compact yarders) supply the payload/distance/timing tables needed to round out our micro-yarder presets. We still need regression-ready JSON plus machine-rate hooks for each system.
          - *Progress 2025‑12‑06:* TN54 is fully wired (CLI preset + JSON), TN39’s footprint/cost curves live in `notes/reference/skyline_small_span_notes.md`, TN173 now delivers end-to-end coverage (structured JSON + CLI presets + machine-rate roles), the FNG73 Hi-Skid truck ships as both a dataset (`data/reference/fpinnovations/fng73_hi_skid.json`) and a CLI skyline model (`--model hi-skid` with optional travel-inclusive reporting), the default harvest-system registry now includes `cable_micro_*` / `cable_micro_hi_skid`, and `data/machine_rates.json` now documents the Hi-Skid owning (9.0 $ depreciation + 2.2 $ interest = 11.2 $/SMH) and operating (33.13 $ wage + 5.2 $ fuel + 16 $ maintenance = 54.33 $/SMH) components derived directly from the Field Note + ADV2N62 wage table. FNCY12/TN258 carries the new monthly support summary, and synthetic tiers continue to sample every `cable_micro_*` preset plus Hi-Skid. Remaining desire: authentic TN258 support timing/payroll logs should they ever surface.
        - [x] **Partial-cut skyline penalties** – ADV11N17 (MPB partial cuts), ADV1N22 (Cariboo small openings), TN199 (Quadra Island second-growth partial cut), SR109 (MASS shelterwood/patch/green-tree), and ADV9N4 (fuel-reduction thinning) are now digitized (`data/reference/fpinnovations/*partial*.json`). `data/reference/partial_cut_profiles.json` captures the derived volume/cost multipliers so the CLI can apply them without reopening the PDFs.
            - `fhops.dataset estimate-skyline` gained ``--partial-cut-profile`` (with telemetry + cost-banner integration) and the skyline harvest systems auto-select the matching profile (`cable_running_tr122_shelterwood`, `cable_partial_tr127_block{1,5}`), replacing the old TR119-only proxies.
            - Docs now carry a dedicated partial-cut profile table plus updated harvest-system notes so analysts can see which presets pull SR109/ADV11N17/TN199/ADV9N4 penalties; tests exercise both the harvest-system defaults and the manual CLI flag.
        - **Eastern/interior skyline benchmarks & manual support costs** – TN173’s five cable-yard case studies plus TN82/TN98/TN106/TN111/TN288/TR2022N18 manual/motor-manual studies give non-coastal payloads, hand-falling costs, and fuel-treatment prescriptions. Prioritise any regression-ready data that lets skyline costing swap in manual falling or moto-manual thinning rates when the helper selects partial cuts or fuel-reduction modes.
      - [ ] **FPInnovations skyline backlog (from reference log “Next Actions”)**
        ...
        - **Road construction + soil-protection backlog (TR28/FNRB3/ADV4N7/ADV15N3)** – TR28 tables PC-1–PC-3 + Appendix V are now digitised in `data/reference/fpinnovations/tr28_subgrade_machines.json` (Cat 235 backhoe, D8H dozer, American 750C, Poclain HC300) with movement, cycle, cost, and roughness fields ready for the roadbuilding cost helper. Docs + CLI now surface the dataset (`fhops dataset tr28-subgrade`), the standalone helper (`fhops dataset estimate-road-cost`), **and** the inline `estimate-cost` add-on (``--road-machine`` + ``--road-length-m``) so planners can bolt TR-28 subgrade estimates directly onto machine-cost runs. Every time the road block is enabled the CLI prints structured soil profiles pulled from `data/reference/soil_protection_profiles.json` (FNRB3 ground-pressure multipliers, ADV4N7 compaction thresholds), and scenarios can keep a ``road_construction`` CSV so `estimate-cost --dataset …` picks the right job via ``--road-job-id`` and carries the same guidance without extra flags. **New:** harvest-system templates + the synthetic generator now auto-inject those `road_construction` rows whenever a skyline/cable preset carries defaults (see `SYSTEM_ROAD_DEFAULTS`), the bundle writer emits `data/road_construction.csv` beside the other tables, and `estimate-cost --telemetry-log …` captures the selected road machine/length/mobilisation flag + soil profiles so downstream costing logs stay synchronized. The published synthetic bundles (`examples/synthetic/{small,medium,large}`) were regenerated with the new pipeline so their `scenario.yaml` files reference the fresh `data/road_construction.csv` tables, keeping docs/benchmarks in sync with the automation. Remaining work: move beyond reminders by threading the ADV15N3 tractor-efficiency + FNRB3/ADV4N7 timing/compaction deltas into actual multipliers once the missing tables are parsed, so road helpers flip penalties automatically instead of static notes.
          - *Update 2025‑12‑06:* `fhops.dataset estimate-cost` now applies the ADV4N7/ADV15N3 penalties automatically—``adv4n7_compaction`` road profiles default to the “some” multiplier (1.15×) and any road entry that uses ``caterpillar_d8h_bulldozer`` inherits the Cat D7R low-speed penalty from ADV15N3. Analysts can override the risk (``--road-compaction-risk``) or force a specific drive (``--road-tractor-drive jd_950j_hydrostatic`` / ``cat_d7e_electric``) and the helper folds those multipliers into both the console output and telemetry summaries, retiring the old “remember to add 16 %” reminder.
        - **Debris carry-back / microsite backlog (TR2017N21)** – document the Sundre debris-return trial so the silviculture module knows the cost/productivity of dragging processing slash back onto 20–45 % slopes (vs. piling/burning) and can quote the observed microsite-density lift when we add slash-management toggles to the scenario generator. No new data sources are coming, so this FPInnovations trial is our only reference for winter desiccation mitigation.
        - Advantage skyline issues – ADV1N5, ADV5N28, ADV7N3, etc. already catalogued with short abstracts (`notes/reference_log.md:13,39,45`). ADV5N28 is fully wired, ADV7N3 now powers `--processor-model adv7n3` (and the skyline harvest-system presets auto-select the deck costs), ADV1N35 ships as `--grapple-yarder-model adv1n35`, ADV1N40 just landed as the `adv1n40` Madill 071 downhill preset, ADV6N7 now backs both the `adv6n7` grapple-skidder helper and the `ground_fb_*` harvest systems (auto-selecting the Caterpillar 535B defaults + loader-support ratio), and ADV2N21’s partial/patch cost matrix lives in `data/reference/fpinnovations/adv2n21_partial_cut.json` (`fhops dataset adv2n21-summary`). ADV1N5 is now mined for salvage guidelines (debarking upgrades, charcoal-dust controls, log sorts, grapple yarding of sensitive slopes, portable milling/in-woods chipping); docs/CLI planning sections reference those cautions, the new `ground_salvage_grapple` / `cable_salvage_grapple` harvest systems bundle the defaults, and `--salvage-processing-mode` lets analysts toggle standard mill vs. portable mill vs. in-woods chipping notes. Next Advantage-specific work: push these salvage flags into the scenario generator/telemetry schema so salvage scheduling knobs live beside the presets.
    ...
    - [x] **Scenario wiring + synthetic coverage** – Teach harvest-system templates and the synthetic generator to auto-populate skyline predictors (span, deflection, lateral distance, payload, latd2) and ensure telemetry/regression tests cover those defaults.
  - ### Helicopter action plan (refresh)
    - [x] **Consolidate FPInnovations heli datasets** – ADV3/4/5/6 (plus the Kamov KA-32A pole-logging case) now live in `data/productivity/helicopter_fpinnovations.json`, grouped by preset ID with flight distance, payload, turn timing, cost/currency metadata, and source pointers. `fhops.dataset helicopter-fpinnovations` lists them (with `--operation-id` for detail) and the CLI defaults/presets pull directly from this JSON.
    - [x] **Cost integration** – Added CPI-aware machine-rate roles for each aircraft (`helicopter_lama`, `_kmax`, `_bell214b`, `_ka32a`, `_s64e_aircrane`) so `--show-costs` and `inspect-machine` surface the FPInnovations Appendix-II splits without manual lookups. The TMY45 role already ties into the Thunderbird work above.
    - [x] **Telemetry & docs** – ``--helicopter-model`` gained a KA-32A option plus a ``--helicopter-preset`` flag (defaults per model when omitted). The CLI banner/telemetry now state which preset seeded the defaults, and docs/harvest-system notes point analysts to the new helper + machine-rate roles.

...
- [ ] **Roadside processors / loaders / hoe chuckers (Labelle et al. 2016/2018; FPDat loader datasets)**
  - [ ] Ingest landing-processor time studies (processing time per log vs. diameter/species) and generic loader cycle-time models. (`notes/reference/Berry, Nick_Final-Dissertation.pdf` is the first modern dataset; older comparisons live in `administrator,+jfe5_2tp04.pdf` (NZ landing processors) and `notes/reference/unbc_15814.pdf` (BC hoe-chucking crews).)
    - [ ] Digitise Berry (2019) per-log cycle times and log-grade splits (Tables 10–12 + Appendix 5) into `data/productivity/processor_berry2019.json` so the helper can optionally break productivity down by grade mix rather than just piece size (only UH + BHS anchors are captured so far; full grade table still pending extraction).
    ...
    - [x] Extract the skid-size vs. delay relationship from Berry Section 5.5 and expose it via `--processor-skid-area-m2`. The CLI now predicts average delay seconds/stem, warns when the landing size is outside the 2.5–3.7k m² study range, and (when the user hasn’t supplied `--processor-delay-multiplier`) auto-scales the utilisation multiplier using the published regression.

...
- [ ] **Roadside processors / loaders / hoe chuckers (Labelle et al. 2016/2018; FPDat loader datasets)**
    - [ ] Ingest landing-processor time studies (processing time per log vs. diameter/species) and generic loader cycle-time models. (`notes/reference/Berry, Nick_Final-Dissertation.pdf` is the first modern dataset; older comparisons live in `administrator,+jfe5_2tp04.pdf` (NZ landing processors) and `notes/reference/unbc_15814.pdf` (BC hoe-chucking crews).)
        - [ ] Digitise Berry (2019) per-log cycle times and log-grade splits (Tables 10–12 + Appendix 5) into `data/productivity/processor_berry2019.json` so the helper can optionally break productivity down by grade mix rather than just piece size. Appendices are currently images in the PDF; need either the original vector source or manual transcription to capture all grade codes (only UH + BHS anchors available today). Appendix 13’s emmeans plot has been digitised into `data/reference/berry2019_log_grade_emmeans.json` for future NZ-specific analysis but is not wired into FHOPS.
        ...
        - [x] Build a TN-46 loader/processor costing helper that feeds the costing CLI (machine rates + $/m³) using the study’s wait-for-truck breakdown so budgeting workflows can pick the Barko preset end-to-end.
            - Status recap:
                - ✅ TN-46 productivity/availability/utilisation encoded in `data/productivity/loader_barko450.json`, and the loader CLI exposes `--loader-model barko450` with the truck-wait warning.
                - ✅ Alternate FPInnovations sources (TR-73, TN-64, TN-51) are catalogued (`notes/reference_log.md`) and their cost-per-shift / $/m³ / $/piece data is already merged into the Barko JSON + CLI output.
            ...
            - Next steps:
                ...
                2. ✅ Added a Barko entry to `data/machine_rates.json` (`role: loader_barko450`) using the TR-45 Appendix II owning/operating/maintenance split (19.88+17.41+19.88 $/SMH in 1979 CAD) and TN-46’s 0.79 utilisation so the inflation helper can compose a Barko-specific rental rate.
                3. ✅ `fhops.dataset estimate-productivity --machine-role loader --loader-model barko450 --show-costs` now calls the Barko-specific machine rate so the CLI/telemetry surface the CPI-inflated $/SMH breakdown (owning/operating/repair) with the TN-46 utilisation default, and supplying `--loader-utilisation` automatically rescales the Barko production + $/m³ outputs to reflect alternate truck supply assumptions.
                4. ✅ `fhops.dataset inspect-machine`/`estimate-cost` recognise the Barko slug: `--machine-role loader_barko450` dumps the CPI-scaled TN-46 rate directly, and selecting `--system ground_fb_loader_liveheel` now auto-swaps the cost-role so budgeting/QA flows pick up the live-heel loader defaults without manual edits.

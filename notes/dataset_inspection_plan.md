  ...
  - ### Skyline action plan (refresh)
    ...
    - [ ] **Skyline costing defaults** – Build CPI-aware machine-rate entries for representative skyline yarders (Madill 009, Cypress 7280C, Skylead C40) plus rigging/crew assumptions, and thread them into `inspect-machine` / `--show-costs` / harvest-system overrides.
      ...
      - [ ] **FPInnovations skyline backlog (from reference log “Next Actions”)**
        ...
        - Advantage skyline issues – ADV1N5, ADV5N28, ADV7N3, etc. already catalogued with short abstracts (`notes/reference_log.md:13,39,45`). ADV5N28 is now fully wired: the JSON above feeds the new `adv5n28-clearcut|adv5n28-shelterwood` grapple-yarder presets, the registry gained matching harvest-system IDs (`cable_running_adv5n28_clearcut|_shelterwood`), and telemetry/`--show-costs` now log the CPI-adjusted skyline vs. heli savings every time those presets are selected. Next step: resume ADV7N3 transcription to cover the summer short-wood decks; ADV1N5 remains qualitative salvage context (no tables).

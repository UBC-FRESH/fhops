...
- [ ] **Skyline yarders / helicopter longline (Aubuchon 1982; Böhm & Kanzian 2023; Arnvik helicopter refs)**
  ...
  - [ ] Add helicopter longline regression(s) (payload vs. cycle time vs. flight distance) for `helicopter_longline` jobs. (Use Aubuchon 1982 rotorcraft cycle equations, Böhm & Kanzian 2023’s review pointers, plus the helicopter-cost references cited in Arnvik’s Appendix 1.)
    - Fresh FPInnovations helicopter datasets now catalogued in `notes/reference_log.md`: `ADV5N13.PDF` (S-64E Aircrane), `ADV5N38.PDF` (Bell 214B variable retention), `ADV4N19.PDF`/`ADV4N33.PDF` (Queen Charlotte & central coast trials), `ADV6N25.PDF` (mixed Lama + K-Max) and `ADV3N19.PDF`/`ADV3N20.PDF` (Skycrane + Kamov pole logging). Use their payload/shift/cost figures to anchor the light/medium/heavy helicopter helper presets. Also capture planning cues from `TR2015N52.PDF` (drone-assisted strawline setup to reduce helicopter mobilization).

  ...
  - ### Skyline action plan (refresh)
    ...
    - [ ] **Skyline costing defaults** – Build CPI-aware machine-rate entries for representative skyline yarders (Madill 009, Cypress 7280C, Skylead C40) plus rigging/crew assumptions, and thread them into `inspect-machine` / `--show-costs` / harvest-system overrides.
      ...
      - [ ] **FPInnovations skyline backlog (from reference log “Next Actions”)**
        ...
        - **Road construction + soil-protection backlog (TR28/FNRB3/ADV4N7/ADV15N3)** – now that we only have the published FPInnovations/FERIC documents to lean on, start digitising the TR28 subgrade cycle/cost tables (Cat 235 backhoe, D8H dozer, American 750C line shovel, Poclain HC300) so we can seed the upcoming roadbuilding cost helper. FNRB3 (1985 D7H field note) and ADV15N3 (2014 bulldozer fuel tests) give us concrete productivity/fuel deltas between mechanical, hydrostatic, and diesel-electric tractors—use them to justify the Cat D8 and Timberjack 450 support ratios now embedded in `grapple_yarder_tmy45`, and to add realistic mobilisation costs for winch-assist anchor dozers. ADV4N7’s compaction/rutting handbook needs to be surfaced in CLI warnings (trail spacing, tyre pressure, thaw-season gating) once the roadbuilding presets land.

...
- [ ] **Roadside processors / loaders / hoe chuckers (Labelle et al. 2016/2018; FPDat loader datasets)**
    - [ ] Ingest landing-processor time studies (processing time per log vs. diameter/species) and generic loader cycle-time models. (`notes/reference/Berry, Nick_Final-Dissertation.pdf` is the first modern dataset; older comparisons live in `administrator,+jfe5_2tp04.pdf` (NZ landing processors) and `notes/reference/unbc_15814.pdf` (BC hoe-chucking crews).)
        - [ ] Digitise Berry (2019) per-log cycle times and log-grade splits (Tables 10–12 + Appendix 5) into `data/productivity/processor_berry2019.json` so the helper can optionally break productivity down by grade mix rather than just piece size. **Reality check:** the thesis stops at Table 9; the only grade-level data lives in Appendix 13 as an image. We have already digitised those emmeans—there is nothing else to transcribe unless FPInnovations releases the raw Kinleith dataset.

    ...
    - [ ] **Skyline costing defaults** – Build CPI-aware machine-rate entries for representative skyline yarders (Madill 009, Cypress 7280C, Skylead C40) plus rigging/crew assumptions, and thread them into `inspect-machine` / `--show-costs` / harvest-system overrides.
      ...
      - [ ] **Skyline data shopping list (FPInnovations Nov 2025 drop)** – new PDFs staged under `notes/reference/fpinnovations/` map to these outstanding needs:
        ...
        - **Small-span skyline regressions** – TN54 (Micro Master yarder), FNG73 (Hi‑Skid short-yarding truck), TN39 (log-sorting logistics), and TN173 (Eastern compact yarders) supply the payload/distance/timing tables needed to round out our micro-yarder presets. We still need regression-ready JSON plus machine-rate hooks for each system.
          - *Progress 2025‑11‑30:* TN54 is fully wired (CLI preset + JSON), TN39’s footprint/cost curves live in `notes/reference/skyline_small_span_notes.md`, and TN173 now delivers end-to-end coverage: structured JSON, skyline CLI presets (`tn173-*`), automatic span warnings, and matching machine-rate entries (`skyline_*_tn173`) so `--show-costs` can cite the Eastern hourly rates. Next steps: (1) finish the FNG73 helper scaffolding (`hi_skid` preset + ownership assumptions); (2) add harvest-system overrides/system-registry hooks for the TN173/FNG73 presets so scenario defaults auto-select the right inputs; (3) move on to TN258/FNCY12 to unlock Thunderbird TMY45 + Mini‑Mak costing and span penalty wiring so the small-span suite covers both western and eastern references.

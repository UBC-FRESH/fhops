...
- **FPInnovations reference batch catalogued**
   ...
   - Task list moving forward:
     ...
     2. [ ] Implement helper functions (inline with `fhops.productivity.cable_logging`) that load the JSON and expose `estimate_tr127_cycle_minutes(block, sd, latd, logs)` / `estimate_tr127_productivity(block, payload_m3, sd, latd, logs)`—blocks 1–4 only require the listed predictors; blocks 5–6 also include `logs`. Enforce the published ranges and emit warnings when queries fall outside them.
     3. [ ] Update `fhops.productivity.__init__` and `fhops.dataset estimate-skyline` to expose the TR127 variants (e.g., `--model tr127-block5`), printing the source + applicable ranges. Add regression tests feeding the Appendix example (Block 5 at SD=365 m, LATD=16 m, LOGS=3 should yield ≈6.39 min cycle time) to validate the helper/CLI path.
     4. [ ] Document the new TR127 reference + helper in `CHANGE_LOG.md` and the data-contract guide (and reiterate that TR112 lacks regression coefficients) so future work doesn’t chase missing data again.

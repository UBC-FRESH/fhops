  ...
  - ### Skyline action plan (refresh)
    ...
    - [ ] **Skyline data shopping list (FPInnovations Nov 2025 drop)** – new PDFs staged under `notes/reference/fpinnovations/` map to these outstanding needs:
      ...
      - [ ] **FPInnovations skyline backlog (from reference log “Next Actions”)**
        ...
        - **Road construction + soil-protection backlog (TR28/FNRB3/ADV4N7/ADV15N3)** – TR28 tables PC-1–PC-3 + Appendix V are now digitised in `data/reference/fpinnovations/tr28_subgrade_machines.json` (Cat 235 backhoe, D8H dozer, American 750C, Poclain HC300) with movement, cycle, cost, and roughness fields ready for the roadbuilding cost helper. Docs + CLI now surface the dataset (`fhops dataset tr28-subgrade`) *and* the first helper command (`fhops dataset estimate-road-cost`) that multiplies the unit cost by a requested road length, inflates it to CAD 2024, and optionally tacks on the mobilisation charge. Remaining work: plug these estimates into the broader road-cost/soil-warning module and line up the follow-on sources (FNRB3 D7H standby fuel, ADV15N3 tractor efficiency, ADV4N7 compaction limits) so we can add warnings/multipliers once the TR28-backed presets are live. FNRB3 review complete: the Caterpillar D7H vs. D7G trial (MacMillan Bloedel, 1985) documents 20–30 % higher productivity for stripping/ditching/ripping, lower ground pressure via the wider/longer undercarriage, and modular repair advantages. No cycle tables yet, but the field note gives enough qualitative data to justify using a D7H surrogate for future winch-assist anchor dozer costs; still need explicit SMH/fuel numbers from other references before wiring the helper.

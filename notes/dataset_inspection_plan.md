...
- [ ] **Roadside processors / loaders / hoe chuckers (Labelle et al. 2016/2018)**
    - [ ] Ingest landing-processor time studies (processing time per log vs. diameter/species) and generic loader cycle-time models. (`notes/reference/Berry, Nick_Final-Dissertation.pdf` is the first modern dataset; older comparisons live in `administrator,+jfe5_2tp04.pdf` (NZ landing processors) and `notes/reference/unbc_15814.pdf` (BC hoe-chucking crews).)
        - [x] Digitise Berry (2019) per-log cycle times and log-grade splits **Reality check:** the thesis stops at Table 9; the only grade-level data lives in Appendix 13 as an image. We have already digitised those emmeans—there is nothing else to transcribe unless FPInnovations releases the raw Kinleith dataset. Remaining work focuses on other landing-processor sources.
        - [x] Exposed the Appendix 13 emmeans via `fhops.dataset berry-log-grades` and `--processor-show-grade-stats` so analysts can reference the digitised grade means ±2σ (with the “image-derived” caveat) without altering the helper equations.
            - [ ] Triage the remaining landing-processor references: summarize what equations/data exist in `administrator,+jfe5_2tp04.pdf` (NZ landing processors) and `notes/reference/unbc_15814.pdf` (BC hoe-chucking) so we know which tables to transcribe.
                - [x] JFE 5(2) (Kellogg & Bettinger) only covers the Timberjack 2518 harvester + FMG 910 forwarder; no landing-processor cycle tables exist beyond the harvester/forwarder regressions already captured in `data/reference/administrator_jfe5_2tp04.json`.
                - [x] UNBC thesis provides manual landing-processing cost tables (Tables 21–22) and hoe-chucking shift summaries (Table 33) with $/m³ by treatment but no mechanized landing-processor cycles; usable data = cost per treatment, equipment rate ($103.48/h), and observed manual processing priorities.
            - [x] Design structured JSON layouts (e.g., `data/reference/unbc_hoe_chucking.json`) capturing scenario metadata, productivity, utilisation, and cost base years.
            - [x] Transcribe the UNBC hoe-chucker processor timings/utilisation into JSON (per treatment) with CPI metadata.
            - [x] Surface the dataset via `fhops.dataset unbc-hoe-chucking` (Rich table + docs/tests) so planners can reference the observed SMH/productivity/$ per m³ when budgeting manual hoe-chucking support.
        - [x] Capture the ADV5N6 loader-forward vs. grapple-yard processor productivity/cost tables (hot vs. cold processing) into `data/productivity/processor_adv5n6.json`, then add a coastal processor helper that warns about stem source (`loader_forwarded` vs. `grapple_yarded`) and shift structure (single vs. double shift). `estimate_processor_productivity_adv5n6` now surfaces those Block B–D scenarios, `fhops.dataset estimate-productivity --machine-role roadside_processor --processor-model adv5n6` exposes the `--processor-stem-source/--processor-processing-mode` switches, docs/tests/reference log all updated, and CLI output reminds users that the $/m³ + utilisation numbers are straight from ADV5N6.
        - [x] Stage TN-166 (Denis D3000 telescopic processor) timing/cost data into JSON so we have an interior BC reference (0.67 min/stem, 44.8 m³/PMH yarded, $2.51–3.13/m³). `data/productivity/processor_tn166.json` now feeds `--processor-model tn166` (with scenario flag `--processor-tn166-scenario`) so analysts can pick grapple-yarded vs. ROW vs. mixed shift averages directly from the CLI.
        - [x] Summarize TN-46 Barko 450 loader utilisation constraints (≈658 m³/shift, 96% availability, 79% utilisation, truck-wait sensitivity) so loader presets can reuse the wait-for-truck multiplier. `data/productivity/loader_barko450.json` now captures the ground-skid vs. yarder blocks, availability/utilisation splits, and per-load volumes for future loader helpers/metadata.
        - [x] Expose the Barko 450 presets in the CLI (`--loader-model barko450` + `--loader-barko-scenario`) so analysts can pull the TN-46 production/utilisation numbers directly from `fhops.dataset estimate-productivity` (docs/tests/telemetry updated to mirror the new helper).
        - [x] Normalize all legacy $ figures (TR-73, TN-46, TN-166, ADV5N6) to 2024 CAD using Statistics Canada CPI (Table 18-10-0005-01). Repository now carries `data/costing/cpi_canada_all_items_2002_100.json` plus an inflation helper so every loader/processor helper auto-inflates its source-year costs.
        - [x] Build a TN-46 loader/processor costing helper that feeds the costing CLI (machine rates + $/m³) using the study’s wait-for-truck breakdown so budgeting workflows can pick the Barko preset end-to-end.
            - Status recap:
                - [x] TN-46 productivity/availability/utilisation encoded in `data/productivity/loader_barko450.json`, and the loader CLI exposes `--loader-model barko450` with the truck-wait warning.
                - [x] Alternate FPInnovations sources (TR-73, TN-64, TN-51) are catalogued (`notes/reference_log.md`) and their cost-per-shift / $/m³ / $/piece data is already merged into the Barko JSON + CLI output.
                - ⏸️ Cost-normalised machine-rate entry still missing: TR-45 Appendix II tables were too garbled to pull the Barko live-heel loader rate directly, so the dedicated costing helper was deferred.
            - Next steps:
                1. [x] Re-extracted TR-45 Appendix II with `pdftotext -layout` and captured the loader cost table (Caterpillar 966C sample, $57.17/SMH in Jan‑1979 CAD with purchase price $165k, 25 % residual, 5‑yr straight-line depreciation, 13 % interest, 10 000 h life, $4.70 fuel, $19.88 maintenance). The appendix doesn’t list a Barko-specific line, so we still need to decide whether to scale this 966C rate to the Barko 450 or pull the TR‑73/TN‑64/TN‑51 cost figures as the dedicated Barko baseline.
                2. [x] Added a Barko entry to `data/machine_rates.json` (`role: loader_barko450`) using the TR-45 Appendix II owning/operating/maintenance split (19.88+17.41+19.88 $/SMH in 1979 CAD) and TN-46’s 0.79 utilisation so the inflation helper can compose a Barko-specific rental rate.
                3. [x] `fhops.dataset estimate-productivity --machine-role loader --loader-model barko450 --show-costs` now calls the Barko-specific machine rate so the CLI/telemetry surface the CPI-inflated $/SMH breakdown (owning/operating/repair) with the TN-46 utilisation default, and supplying `--loader-utilisation` automatically rescales the Barko production + $/m³ outputs to reflect alternate truck supply assumptions.
                4. [x] `fhops.dataset inspect-machine`/`estimate-cost` recognise the Barko slug: `--machine-role loader_barko450` dumps the CPI-scaled TN-46 rate directly, and selecting `--system ground_fb_loader_liveheel` now auto-swaps the cost-role so budgeting/QA flows pick up the live-heel loader defaults without manual edits.
        - [x] Catalog legacy Caterpillar DL221 processor (TN-103) cycle/cost data and Link-Belt TJ90 processor stats from TR-87/TR-106 for future “historic/legacy” helpers or cost validators.
            - `data/productivity/processor_tn103.json` now stores Area A (feller-bunched windrows), Area B (hand-felled windrows), the observed combined average, and the FERIC 73 % utilisation scenario. `--processor-model tn103` with `--processor-tn103-scenario [...]` exposes those presets in the CLI, pulling costs forward to 2024 CAD via CPI.
            - `data/productivity/processor_tr87.json` captures the TJ90 day/night, combined, and wait-adjusted system scenarios from FERIC TR-87. `--processor-model tr87` with `--processor-tr87-scenario [...]` now emits those productivity/cost tables (inflated to 2024 CAD) so the roadside CTL case study can be replayed inside FHOPS.
            - `data/productivity/processor_tr106.json` covers the TR-106 partial-cut processors (Case 1187B + Denis stroke delimber Oct–Nov vs. Feb, plus Steyr KP40 carriers on Cat 225/Link-Belt L-2800/Cat EL180). `--processor-model tr106` together with `--processor-tr106-scenario [...]` now returns the published PMH/net-PMH, cycle minutes, utilisation, and CPI-scaled cost metadata directly in the CLI/telemetry/tests/docs, giving us an interior lodgepole pine preset to pair with the coastal TR-87 helper.
        - [ ] Integrate the newly catalogued peer-reviewed landing-processor/loader references (notes/reference_log.md rows 227–236) into actionable helpers/flags.
            - [x] Built the “log sort complexity” helper: `data/productivity/processor_visser2015.json` holds the 5/9/12/15-sort curves and `--processor-model visser2015` + `--processor-log-sorts` now interpolate the delay-free m³/PMH, apply the CLI utilisation multiplier, and print the study’s value-per-PMH deltas (with NZ provenance warnings) alongside the ADV5N6 presets.
            - [x] Extended the loader helper/CLI docs with the Kizha et al. (2020) preset (`--loader-model kizha2020` + `--loader-hot-cold-mode`). CLI output now reports the 55 % vs. 7 % utilisation split, dominant delay contributors, and the $/PMH penalty so truck-supply sizing decisions can weigh hot vs. cold biomass loading.
            - [x] Added the processor carrier flag (`--processor-carrier purpose_built|excavator`) backed by Magagnotti/Nakagawa/Borz/Bertone datasets so Berry (2019) outputs reflect excavator vs. dedicated carriers (utilisation defaults, productivity penalty, fuel notes) and CLI messaging cites the supporting references.
            - [x] Build a “tractor processor” preset leveraging the HYPRO 775 studies (Castro Pérez 2020 + Zurita 2021) capturing 45 s cycles, 21 L/h, and ergonomic constraints so thin-wood landing operations have an entry-level helper.
                - `data/productivity/processor_hypro775.json` now holds the Castro/Zurita timing + ergonomic metrics (cycle seconds, gross/net trees per hour, fuel 21 L/h, noise ≈85 dB(A), cardio workload 41 % max HR).
                - `estimate_processor_productivity_hypro775` returns the structured result (with utilisation or custom delay multiplier) and `--processor-model hypro775` renders the preset in the CLI/telemetry/tests/docs, mirroring the CPI messaging/warnings used on other helpers.
            - [x] New EBSCO copy of Spinelli et al. (2010) is text-searchable; captured Eq. 1–6 (move/brush/grab/fell/process) + validation tables so we can fold the Italian productivity standards into helper presets without extra OCR work.
            - [x] Build the “Alpine excavator processor” preset off Bertone & Manzone (2025) so we can model cable-landing processors with heavy delay exposure.
                - `data/productivity/processor_bertone2025.json` now stores the DBH/height/log regression, fuel (16.5 L/SMH), cost (€103.46/SMH, base 2023 EUR), and the observed 0.57 utilisation multiplier.
                - `estimate_processor_productivity_bertone2025` + `--processor-model bertone2025` consume DBH/height/log inputs (via `--processor-dbh-cm`, `--processor-tree-height-m`, `--processor-logs-per-tree`, `--processor-piece-size-m3`) and report delay-free vs. SMH productivity alongside the Euro cost/fuel stats; docs/tests/CLI updated.
            - [x] Parse TR-45 Appendix II machine-rate tables to recover the Cat 966C loader, blended grapple skidder, and Cat D7G bulldozer costs.
                - `notes/reference/tr45_appendixII.txt` now holds the transcription; `data/costing/tr45_appendixii_1979.json` captures the raw parameters (I, R, wages, fuel, maintenance, utilisation).
                - `data/machine_rates.json` gained explicit entries for `loader_cat966c_tr45`, `skidder_tr45`, and `bulldozer_tr45`, and the Barko loader rate now uses the Cat 966C owning/operating split (19.88 + 37.29 $/SMH, 1979 CAD) before CPI inflation.
            ...
            - [ ] Transcribe Nakagawa et al. (2010) (Hokkaido excavator processor) cycle regressions to give us a shovel-processor helper keyed to DBH.
                - Published equation: productivity (m³/PMH) = 0.363·DBH^1.116 with per-element regressions (delimbing, cross-cut, stacking) vs. DBH; study mean ≈10.8 m³/PMH with 0.4–0.5 m³ trees.
                - Capture the regression coefficients + utilisation/fuel info so the CLI can surface a “Japanese excavator landing processor” preset for small/medium stems or as a fallback when tree-dimension inputs exist but log counts do not.
                - `data/productivity/processor_spinelli2010.json` documents the equations + accessory/delay ratios and `estimate_processor_productivity_spinelli2010(...)` now drives `--processor-model spinelli2010`, complete with operation/stand/carrier/head/species toggles and automated delay adjustments for both harvest and landing modes (docs/tests/CLI updated).
    ...
    - [ ] Extract the skid-size vs. delay relationship from Berry Section 5.5 and evaluate whether it warrants an optional `skid_size_m3` input (or just document it as a utilisation warning).
    - [ ] Mine `administrator,+jfe5_2tp04.pdf` for the NZ landing-processor equations (stems/hr vs. DBH/log sorts) and queue them as additional helper variants (likely `processor_model nz_landing_2004`).
    ...
    - [ ] Pull the UNBC hoe-chucking processor timing data (`notes/reference/unbc_15814.pdf`) and decide if it fits as another helper or as documentation for steep/interior utilisation multipliers.

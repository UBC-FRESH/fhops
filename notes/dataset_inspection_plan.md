...
- [ ] **Skyline yarders / helicopter longline (Aubuchon 1982; Böhm & Kanzian 2023; Arnvik helicopter refs)**
  ...
  - ### Skyline action plan (refresh)
    ...
    - [ ] **Skyline costing defaults** – Build CPI-aware machine-rate entries for representative skyline yarders (Madill 009, Cypress 7280C, Skylead C40) plus rigging/crew assumptions, and thread them into `inspect-machine` / `--show-costs` / harvest-system overrides.
      ...
      - [ ] **Next action (Item 5)** – Thunderbird TMY45 + Mini-Mak (FNCY12/TN258) machine-rate entry **blocked**: the FNCY12 field note and TN-258 load-study capture productivity, rigging geometry, and skyline/guyline tensions but provide no owning/operating breakdown. Wish list: locate a costing appendix for the Washington 78SL/TMY45 combo or re-create the rate using historical bookkeeping (hourly wages, fuel, rigging depreciation) before wiring `grapple_yarder_tmy45`. Until then, harvest-system presets will keep referencing the Skylead entry.
      ...
      - [ ] **Skyline data shopping list (FPInnovations Nov 2025 drop)** – new PDFs staged under `notes/reference/fpinnovations/` map to these outstanding needs:
        - **TMY‑45 costing (FNCY12/TN258 addenda)** – still no owning/operating split for the Thunderbird TMY‑45 + Mini‑Mak II combo; comb TN54 (Model 9 Micro Master yarder) and TN173 (Eastern Canada cable trials) for comparable rate tables or citations referencing TMY‑45 bookkeeping before wiring `grapple_yarder_tmy45`.

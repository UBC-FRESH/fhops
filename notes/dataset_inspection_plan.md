...
- [ ] **Roadside processors / loaders / hoe chuckers (Labelle et al. 2016/2018)**
    - [ ] Ingest landing-processor time studies (processing time per log vs. diameter/species) and generic loader cycle-time models. (`notes/reference/Berry, Nick_Final-Dissertation.pdf` is the first modern dataset; older comparisons live in `administrator,+jfe5_2tp04.pdf` (NZ landing processors) and `notes/reference/unbc_15814.pdf` (BC hoe-chucking crews).)
        ...
        - [ ] Integrate the newly catalogued peer-reviewed landing-processor/loader references (notes/reference_log.md rows 227–236) into actionable helpers/flags.
            1. [ ] Build a “log sort complexity” option (e.g., `--processor-log-sorts 5|9|12|15`) grounded in Visser & Tolan (2015) so analysts can quantify the 10 % productivity penalty (≈91 → 82 m³/PMH at 2 m³ piece size) and see the associated value-per-PMH shifts. Warn that the data come from NZ cable-yarder radiata landings but closely resemble ADV5N6 operations.
            2. [ ] Extend the loader helper/CLI docs with an explicit `--loader-mode hot|cold` switch tied to the Kizha et al. (2020) biomass study (Thunderbird 840W loader, 70 % utilisation hot vs. cold delays dominated by truck/bin shortages). Output should surface which machine is the bottleneck and add delay warnings so users can size truck fleets accordingly.
            3. [ ] Introduce an “excavator-based processor” carrier flag that applies Magagnotti et al. (2017) utilisation/fuel penalties, Nakagawa et al. (2010) DBH→cycle regressions, Borz et al. (2023) landing-harvester fuel/cost stats, and Bertone & Manzone (2025) yarder-wait delays to differentiate shovel/excavator processors from purpose-built bases inside the helper/CLI/docs/tests.
            4. [ ] Decide whether the Hypro 775 double-grip dataset (Castro Pérez et al. 2020) becomes its own “tractor processor” helper for thinning support or just informs documentation/multipliers for compact landings.
            5. [ ] `Productivity_Standards_for_Har.pdf` is currently image-only—queue an OCR/clean-copy task so we can mine any processor tables before promising additional presets based on it.
        - [ ] Capture the Labelle & Huß (2018) automatic bucking vs. quality bucking deltas (≈+6 m³/PMH, +3–5 €/m³) into `data/reference/processor_labelle_huss2018.json` so the processor CLI can expose an optional `--automatic-bucking` multiplier.
            1. **Prioritisation pass** *(done)* – chose the Labelle & Huß multiplier as the next increment (TN-46 loader costing + FPDat placeholders stay queued) and locked scope to “multiplier toggle + telemetry/docs”.
            2. **Data extraction** *(done)* – `data/reference/processor_labelle_huss2018.json` now stores the published PMH₀ and €/m³ values for manual vs. automatic treatments (Table 8/10) plus base-year metadata so the helper can consume the 12.4 % productivity uplift or €3.3/m³ revenue bump directly.
            3. **Dataset + helper updates** *(done)* – helpers now accept an optional `automatic_bucking_multiplier` parameter, load the Labelle & Huß dataset, and apply the +12.4 % delay-free uplift when requested.
            4. **CLI/docs/tests** *(done)* – added `--processor-automatic-bucking`, documented the behaviour (docs + changelog), and extended the CLI/unit tests to assert that the multiplier path changes the reported productivity and prints the Silva Fennica citation/revenue note.

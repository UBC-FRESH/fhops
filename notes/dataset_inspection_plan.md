  ...
  - ### Skyline action plan (refresh)
    ...
    - [ ] **Skyline costing defaults** – Build CPI-aware machine-rate entries for representative skyline yarders (Madill 009, Cypress 7280C, Skylead C40) plus rigging/crew assumptions, and thread them into `inspect-machine` / `--show-costs` / harvest-system overrides.
      ...
      - [ ] **Skyline data shopping list (FPInnovations Nov 2025 drop)** – new PDFs staged under `notes/reference/fpinnovations/` map to these outstanding needs:
        - **TMY‑45 costing (FNCY12/TN258 addenda)** – owning/operating split reconstructed (see `grapple_yarder_tmy45`), but we still need the TN258 intermediate-support timing sheets and any Jay-Vee Yarding payroll/fuel ledgers so the Cat D8 + Timberjack 450 standby costs (and true IWA wage references) can replace the current LeDoux-scaled assumptions. Also need to push the TMY45 attachment assumptions into harvest-system/scenario presets once the dozer/skidder time is quantified.

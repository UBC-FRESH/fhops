  ...
  - ### Skyline action plan (refresh)
    ...
    - [ ] **Skyline costing defaults** – Build CPI-aware machine-rate entries for representative skyline yarders (Madill 009, Cypress 7280C, Skylead C40) plus rigging/crew assumptions, and thread them into `inspect-machine` / `--show-costs` / harvest-system overrides.
      ...
      - [ ] **LeDoux (1984) residue yarding regressions (USDA NE RP)** – `notes/reference/ne_1984_ledoux_002.pdf` digitizes regression equations + hourly costs for four skyline systems (Skagit BU-94 shotgun/highlead, Washington 208E, Thunderbird TMY-45). Plan:
        - [ ] Transcribe Table 3 regression formulas (convert `ft`/`ft³` units to metric) and stash them under a new data note so we can drive TMY45 productivity defaults when FNCY presets arrive.
        - [ ] Pull the Table 4 hourly costs (1984 USD, new-equipment assumption) into `data/machine_rates.json` as interim machine-rate entries (record CPI and currency conversions; call out that these are US-based residue logs, not BC skyline studies).
        - [ ] Document outstanding wishes (BC wage tables, residue-specific utilisation penalties, inflation factors) so we can tighten the rate once a Canadian costing appendix surfaces.

...
- [ ] **Roadside processors / loaders / hoe chuckers (Labelle et al. 2016/2018)**
    - [ ] Ingest landing-processor time studies (processing time per log vs. diameter/species) and generic loader cycle-time models. (`notes/reference/Berry, Nick_Final-Dissertation.pdf` is the first modern dataset; older comparisons live in `administrator,+jfe5_2tp04.pdf` (NZ landing processors) and `notes/reference/unbc_15814.pdf` (BC hoe-chucking crews).)
        ...
        - [ ] Integrate the newly catalogued peer-reviewed landing-processor/loader references (notes/reference_log.md rows 227–236) into actionable helpers/flags.
            1. [x] Built the “log sort complexity” helper: `data/productivity/processor_visser2015.json` holds the 5/9/12/15-sort curves and `--processor-model visser2015` + `--processor-log-sorts` now interpolate the delay-free m³/PMH, apply the CLI utilisation multiplier, and print the study’s value-per-PMH deltas (with NZ provenance warnings) alongside the ADV5N6 presets.
            2. [x] Extended the loader helper/CLI docs with the Kizha et al. (2020) preset (`--loader-model kizha2020` + `--loader-hot-cold-mode`). CLI output now reports the 55 % vs. 7 % utilisation split, dominant delay contributors, and the $/PMH penalty so truck-supply sizing decisions can weigh hot vs. cold biomass loading.
            3. [x] Added the processor carrier flag (`--processor-carrier purpose_built|excavator`) backed by Magagnotti/Nakagawa/Borz/Bertone datasets so Berry (2019) outputs reflect excavator vs. dedicated carriers (utilisation defaults, productivity penalty, fuel notes) and CLI messaging cites the supporting references.
            4. [ ] GNSS-derived skidder speeds (Zurita & Borz 2025) provide fresh travel-speed distributions for cable skidders vs. farm tractors—use them to refine grapple-skidder helpers (optional multiplier/override for Han 2018) and document recommended default speeds.
            5. [ ] Build a “tractor processor” preset leveraging the HYPRO 775 studies (Castro Pérez 2020 + Zurita 2021) capturing 45 s cycles, 21 L/h, and ergonomic constraints so thin-wood landing operations have an entry-level helper.
            5. [x] New EBSCO copy of Spinelli et al. (2010) is text-searchable; captured Eq. 1–6 (move/brush/grab/fell/process) + validation tables so we can fold the Italian productivity standards into helper presets without extra OCR work.

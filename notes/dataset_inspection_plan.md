  ...
  - ### Skyline action plan (refresh)
    ...
    - [ ] **Skyline costing defaults** – Build CPI-aware machine-rate entries for representative skyline yarders (Madill 009, Cypress 7280C, Skylead C40) plus rigging/crew assumptions, and thread them into `inspect-machine` / `--show-costs` / harvest-system overrides.
      ...
      - [ ] **FPInnovations skyline backlog (from reference log “Next Actions”)**
        - Cable Yarding (FNCY series) – References logged (`notes/reference_log.md:120` onward) for FNCY1/4/10/11/12/17. FNCY1 (Igland-Jones Mini-Alp on Timberjack 330) reviewed: downhill yarding ~85 m³/shift with three-person yarding crew + shared skidder/bucker, uphill ~113 m³/shift using a Koller carriage. No regression tables but provides payload/crew/layout notes for micro-yarders. FNCY4 (radio-controlled chokers on a Washington 118 swing yarder) reviewed: reports 280–340 m³/day uphill grapple yarding with a four-person crew, landing release time cut to 20 s, and battery/service lessons—useful for skyline efficiency assumptions. FNCY10 (Eastern Canada equipment review) logged: specs/crew sizes for small skyline/highlead yarders (Christie, EcoLogger, Smith Timbermaster, Télétransporteur); planning reference only. Remaining FNCY entries (11/12/17) still need triage.
        - Advantage skyline issues – ADV1N5, ADV5N28, ADV7N3, etc. already catalogued with short abstracts (`notes/reference_log.md:13,39,45`). ADV5N28 reviewed: provides full productivity/cost breakdowns for Madill 071 + Acme 200 motorized carriage substituting heli harvest on two blocks (clearcut with reserves vs. irregular shelterwood), plus falling/logging/block layout lessons and machine-cost appendix (Appendix II). Need to digitise those tables (yarding distances 375–725 m, intermediate supports, cross-stream corridors) and compare the adjusted skyline costs ($47.68/m³ clearcut, $69.30/m³ partial) against helicopter $60/m³ baseline. ADV1N5 scanned: salvage logging focus (grapple yarding burned timber, portable milling, in-woods chipping) with useful qualitative notes but no regression tables—keep as operational reference for post-fire skyline/grapple planning. Remaining Advantage references still unmined.

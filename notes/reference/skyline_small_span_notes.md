...
- Cost validation (2025‑12‑06): Re-ran the Field Note assumptions so the `skyline_hi_skid` rental rate is reproducible—depreciation = (CAD 60 k − 10 % salvage)/(5 years × 1 200 SMH) = **9.0 $/SMH**, interest/insurance = 8 % of the average investment (**2.2 $/SMH**), total owning **11.2 $/SMH**. Operating = ADV2N62 June 15 1999 IWA wage (24.01 $/h +38 % = **33.13 $/SMH**) + diesel (8 L/h × 0.65 $/L = **5.2 $/SMH**) + **16 $/SMH** for tires/hydraulics/rigging, yielding **54.33 $/SMH** pre-CPI. These values now sit directly in `data/machine_rates.json` so future CPI updates can trace the inputs without revisiting external invoices.
...
- Dataset: `data/reference/fpinnovations/fncy12_tmy45_mini_mak.json` now captures the equipment specs, rigging geometry, lateral limits, skyline/guyline tensions (from TN-258), and month-by-month productivity so long-span/intermediate-support presets can consume these numbers without re-opening the PDFs. `scripts/fncy12_support_split.py` prints the July “no support” vs. Aug–Oct “support active” productivity split (17 shifts/170 SMH @ 102.5 m³/shift vs. 50.5 shifts/505 SMH @ 166.3 m³/shift), and Table 3’s man-day productivity makes the crew delta explicit: Thunderbird TMY45 at 5.5-person crew → 32.1 m³/man-day, versus 3-person Skylead crews at 45–52 m³/man-day. Even after removing the rigging-only crew, the Thunderbird man-day productivity only rises to 44.2 m³/md, underscoring the labour tax of intermediate supports.
- Dataset update (2025‑12‑06): Added ``support_monthly_summary`` to the JSON so each month now records productive shifts, yarder SMH, and the inferred Cat D8/Timberjack SMH (ratios applied only to the Aug–Oct “supports installed” months and scaled by each month’s share of support SMH). Buckets summarise the support vs. non-support periods, giving the CLI/telemetry a reproducible split even though TN258 never published stopwatch-based support utilisation.
...
- Support utilisation (derived from Table 3 + 23 % support share): Comparing the Thunderbird crew (5.5 workers, 32.1 m³/md) against the Skylead reference (3 workers, 45–52 m³/md) yields an extra 2.5 man-days per shift. Scaling by the 23 % support-area fraction and the TN157 backspar duty share (≈0.58) gives **0.3335 Cat D8 SMH per yarder SMH** and **0.2415 Timberjack 450 SMH per yarder SMH**. These values now live in `data/reference/fpinnovations/fncy12_tmy45_mini_mak.json` and feed the TMY45 preset so we no longer rely on the TN157 proxies. `scripts/fncy12_support_split.py` prints both the productivity split and the derived ratios (see output above). *See `docs/reference/harvest_systems.rst` (“TN-258 / FNCY12 support utilisation”) for the condensed documentation cross-link that downstream readers will use.*
- ADV15N3 + ADV4N7 penalty wiring (2025-12-04): The new `data/reference/fpinnovations/adv15n3_support.json` captures the Caterpillar D7R vs. John Deere 950J vs. Cat D7E fuel curves (Table 2/4). When skyline presets inject the TR-28 road helper, `fhops dataset estimate-cost` uses the D7R low-speed penalty (25.78 / 22.14 ≈ **1.16×** more litres per hour) whenever the road machine slug maps to a D8/D7 tractor and the ADV4N7 soil profile reports “some” or “high” risk. Compaction behaviour from `data/reference/fpinnovations/adv4n7_compaction.json` translates Table 3 guidance into default multipliers: **low** = 1.0 (operate normally), **some** = 1.15 (limit passes to ≤3 and shrink loads), **high** = 1.4 (slash mats/relocate). The CLI prints a support-penalty banner and telemetry logs both multipliers so downstream costing can audit the assumptions without reopening the handbook or ADV report.
...
- Remaining wish list: secure an authentic 1992 BC payroll/fuel/repair worksheet for the Thunderbird fleet plus any Jay-Vee Yarding accounting that allocates intermediate-support moves to the D8 vs. rigging crew. The TN258 manuscript still omits intermediate-support setup/move timing and machine utilisation minutes, so we cannot validate the derived 0.3335/0.2415 ratios against stopwatch data—only the man-day inference described above.
- Intermediate-support timing cross-check (2025‑11‑30): Re-read TN258 (pdftotext excerpt above) and confirmed it only reports static/dynamic line tensions (Tables 1–2) plus rigging geometry—no Cat D8/Timberjack utilisation minutes or per-support cycle timing. That’s why the current presets rely on the FNCY12 man-day deltas (rather than direct time-study logs) to populate the support-machine SMH per yarder SMH fields. Keep this note so future readers know TN258 cannot close the loop without external payroll logs.
- TN157 support-ratio sanity check (2025‑12‑01): recomputed the TN157 road-change minutes ÷ yarding minutes for all seven cases to anchor the backspar vs. trail-support split used in the FNCY12 derivation. Ratios ranged 0.085–0.508 with a mean of 0.247 and median 0.224 (Q1 = 0.137, Q3 = 0.333). Backspar-specific work (Hitachi UH14) consumed ≈58 % of total road-change minutes overall, swinging from 5 % on the shortest corridors to 99 % on the longest partial cuts. These stats now drive the 58 % / 42 % split between the Cat D8 and Timberjack helpers rather than serving as direct support ratios. Reference table:
...
- **TN258 – Thunderbird TMY45 + Mini-Mak II:** costing gap is closed (see `grapple_yarder_tmy45`), but we still need published intermediate-support timing + D8/Timberjack utilisation minutes so span-penalty wiring can distinguish “supports up” vs. “no supports” cycles. Until those logs surface, the Cat D8/Timberjack ratios remain inference-based (man-day delta × support-area share × TN157 backspar split) rather than stopwatch data.
...
- [x] **TN258 intermediate-support timing** – Verified (via TN258 text) that the pilot study only presents skyline/guyline tension observations and structural analysis; no intermediate-support timing or support-machine utilisation exists. Because of that gap, the support ratios now rely on the FNCY12 man-day delta + TN157 backspar-share split documented in `fncy12_tmy45_mini_mak.json` (and referenced in `scripts/fncy12_support_split.py`) until authentic payroll sheets surface.

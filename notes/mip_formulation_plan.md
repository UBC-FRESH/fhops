...
- [x] Benchmark harness parity: `run_benchmark_suite(..., include_mip=True)` now builds the operational bundle and calls `solve_operational_milp`, matching the CLI implementation. The helper maps legacy driver names onto solver candidates (`auto` ⇒ `gurobi` then `highs`) and records solver status/termination so telemetry stays informative. `tests/test_benchmark_harness.py::test_benchmark_suite_tiny7` was flipped back to `include_mip=True` and now asserts both the SA + MIP rows (plus the emitted `mip_assignments.csv`) to keep the regression coverage alive.
...
- [x] Mirror the staged-inventory changes inside playback/KPI aggregation (`assignments_to_records` + `compute_kpis`) so sequencing violation counts, staged production, and completed-volume tallies all reference the same logic.
  - [x] Normalised KPI staged-volume bookkeeping so `compute_kpis` clamps near-zero residuals to zero and re-derives `total_production` from block totals. This keeps the day/shift aggregates and KPI snapshots perfectly aligned (no more 1e-12 drift in `tests/test_playback_aggregates.py`).
  - [x] Once the evaluator/repair/watch paths align, refresh the regression assets: reran `fhops solve-heur examples/tiny7/scenario.yaml --iters 200 --out tmp/tiny7_sa_refresh.csv --watch-debug` and `fhops solve-heur examples/med42/scenario.yaml --iters 500 --out tmp/med42_sa_refresh.csv --watch-debug`, regenerated the benchmark JSON summaries + KPI/playback fixtures, and logged the command cadence in the changelog so the regression suite has a clean slate.
...
- [ ] **Fixture + regression refresh for loader buffers (2025-11-30)**
  - [x] Re-run deterministic playback exports for tiny7/med42 (`fhops eval-playback ... --shift-out --day-out --shift-parquet --day-parquet`) using the new MILP assignments so sequencing counters match the tightened loader staging.
  - [x] Recompute deterministic/stochastic KPI snapshots (`tests/fixtures/kpi/*.json`) via `compute_kpis` / `run_stochastic_playback` so regression tests lock onto the new violation totals.
  - [x] Refresh benchmark suite fixtures (`tests/fixtures/benchmarks/tiny7_sa.json`, `med42_sa.json`, `large84_sa.json`) by rerunning `run_benchmark_suite` with the updated heuristics (SA-only, include_mip=True) so docs/tests reference the new objectives/gaps.
  - [x] Update regression baselines in `tests/fixtures/regression/baseline.yaml` once SA/Tabu runs on the toy scenario stabilise with the loader buffers (expect different mobilisation and objective values). *(Values happen to match the prior baseline but were regenerated to prove parity.)*
  - [x] After each fixture refresh, run the relevant pytest slices (`tests/test_benchmark_harness.py -k tiny7`, `tests/test_regression_integration.py`, `tests/test_playback.py`) to confirm sequencing counts/objectives align before attempting a full suite.
...
- [x] Tests/docs: add a small preset fixture (`tests/fixtures/presets/tiny7_explore.yaml`) plus a regression in `tests/heuristics/test_sa_batch.py` that exercises the preset to ensure objective weights stay deterministic. Snapshot lives alongside the CLI docs/change log now that the file is back in-tree.

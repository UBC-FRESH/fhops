...
.. list-table::
   ...
   * - ``blocks.csv``
     ...
     - Optional: ``earliest_start``/``latest_finish`` (defaults 1 / ``num_days``); stand metrics (``avg_stem_size_m3``, ``volume_per_ha_m3``, ``stem_density_per_ha``, ``ground_slope_percent``) now supported and default to Lahrsen (2025) BC ranges in synthetic bundles.
    * - ``machines.csv``
      - ``id``

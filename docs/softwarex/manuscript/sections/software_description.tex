...
\textbf{Automation.} FHOPS users reproduce the experiments by chaining the CLI commands described above: generate or load a scenario, run \texttt{fhops bench suite} (optionally with \texttt{FHOPS\_ASSETS\_FAST=1} for shorter smoke tests), invoke the tuning runner, and replay results with \texttt{fhops playback}. Each command accepts \texttt{--out-dir} and \texttt{--telemetry-log} arguments, so the resulting summaries, telemetry, and KPI tables can be archived or versioned alongside a release tag (e.g., \texttt{v1.0.0-beta1}). This workflow satisfies the SoftwareX reproducibility guidance without relying on manuscript-specific tooling.

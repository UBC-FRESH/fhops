...
1. **Validate or snapshot scenarios.** Run ``fhops dataset validate <scenario.yaml>`` for each bundle (e.g., ``examples/minitoy`` and ``examples/med42``) and, when needed, snapshot synthetic tiers via ``fhops synth generate out/synthetic_{small,medium,large}`` with fixed RNG seeds.

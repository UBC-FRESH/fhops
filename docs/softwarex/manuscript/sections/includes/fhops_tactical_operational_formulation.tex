...
below mirror the implemented Pyomo model in \texttt{fhops.model.milp.tactical\_operational.build\_tactical\_operational\_model} and the validated contract in
\texttt{fhops.planning.tactical\_operational.models.TacticalOperationalScenario}.

\textbf{Problem statement.} Given planning units, period calendars, product yields, harvest-system options, facility demand envelopes, transport arcs, external
...
When demand rows include value per m\(^3\), \texttt{max\_discounted\_profit} maximizes

\[
\sum_{f,p,t}d_tv_{f,p,t}C_{f,p,t}-\text{Cost}.
\]

\texttt{max\_npv} additionally adds declared final-period terminal inventory value:
...
\bottomrule\noalign{}
\endlastfoot
Harvest upper bound & \texttt{model.harvest\_upper} & \texttt{HarvestSystemOption.max\_area\_ha} and block operable area \\
Semi-continuous minimum cut & \texttt{model.harvest\_lower} & \texttt{HarvestSystemOption.min\_area\_ha} \\
Whole-block mode & \texttt{model.whole\_block} & \texttt{PlanningUnit.operable\_area\_ha} \\
Productivity cap & \texttt{model.productivity\_cap} & \texttt{HarvestSystemOption.productivity\_m3\_per\_period} \\
Product conversion & \texttt{model.product\_conversion} & \texttt{PlanningUnit.product\_yields\_m3\_per\_ha} \\
Block area balance & \texttt{model.block\_area} & \texttt{PlanningUnit.operable\_area\_ha} \\
Fleet capacity and acquisition & \texttt{model.fleet\_capacity}, \texttt{model.fleet\_units}, \texttt{model.fleet\_option\_upper} & \texttt{FleetCapacity} and
\texttt{FleetOption} \\
Flow supply & \texttt{model.flow\_supply} & \texttt{TransportArc} and \texttt{product\_volume} \\
Arc capacity & \texttt{model.arc\_capacity} & \texttt{TransportArc.capacity\_m3} \\
Purchase bounds & \texttt{model.purchase\_lower}, \texttt{model.purchase\_upper} & \texttt{ExternalSupply} \\
Consumption bounds/targets & \texttt{model.consumption\_lower}, \texttt{model.consumption\_target}, \texttt{model.consumption\_upper} &
\texttt{FacilityDemand} \\
Inventory balance & \texttt{model.inventory\_balance} & \texttt{InitialInventory}, flows, purchases, consumption \\
Road build/availability/dependencies/access/capacity & \texttt{model.road\_build}, \texttt{model.road\_available}, \texttt{model.road\_build\_timing},
\texttt{model.road\_availability}, \texttt{model.road\_build\_once}, \texttt{model.road\_dependencies}, \texttt{model.road\_access},
\texttt{model.road\_capacity} & \texttt{RoadProject}, \texttt{RoadDependency}, \texttt{BlockRoadAccess} \\
Silviculture fulfillment & \texttt{model.silviculture\_area}, \texttt{model.silviculture\_fulfillment} & \texttt{SilvicultureTransition} \\
Objective profiles & \texttt{model.objective} & \texttt{Economics.objective\_profile}, costs, values, discount factors \\
Bundle replay and telemetry & \texttt{tactical\_bundle\_to\_dict(...)}, \texttt{tactical\_bundle\_from\_dict(...)},
\texttt{solve\_tactical\_operational\_milp(...)} & \texttt{fhops.model.milp.tactical\_operational} \\
\end{longtable}

This formulation is the canonical mathematical reference for the FHOPS tactical--operational MILP. Generated TeX/RST outputs are derived artifacts and should

...
\begin{itemize}
  \item Two published reference scenarios (\texttt{examples/tiny7} and \texttt{examples/med42}) that mirror common BC operational contexts (ground-based vs. mixed skyline/tethered systems).
...
We execute \texttt{fhops bench suite} across tiny7, med42, and synthetic-small scenarios with SA, ILS, and Tabu heuristics plus optional compare presets (\texttt{--compare-preset diversify --compare-preset mobilisation}). Each run produces:
\begin{itemize}
...
\begin{itemize}
  \item \textbf{Tiny7}: SA’s default preset reaches 15.5 production units in $1.45\,$s with 17 assignments, ILS improves the objective to 23.0 in $2.75\,$s with eight assignments, and Tabu’s diversification run now requires $22\,$s (objective 10.0, three assignments) rather than terminating instantly.
...
\begin{itemize}
  \item \textbf{Table~1} – Solver performance summary for tiny7, med42, synthetic-small (derived from the \texttt{fhops bench suite} outputs).

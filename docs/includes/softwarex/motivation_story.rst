...
questions. Jaffray et al. (2025, submitted to the *International Journal
of Forest Engineering*) catalogue the recurring pain points: one-off
solver integrations, weak telemetry, limited robustness testing, and
...
1. **Open, reusable tooling.** FHOPS publishes its data contract, CLI,
   ...
   same scenarios, swap heuristics, and contribute modules without
...
3. **Robust evaluation + extensibility.** FHOPS layers stochastic
   ...
   Those evaluation hooks also set up Rosalia’s thesis Chapter 2: she
   retains the full BC case studies (two–three tenures) and uses FHOPS
   as the engine, while this SoftwareX paper focuses on the platform
   itself.

..

   Reuse plan: exporter script will render this Markdown into
   ``sections/includes/motivation_story.tex`` for the manuscript and
   ``docs/overview_shared_motivation.rst`` for Sphinx so the same

...
- **Simulated annealing** remains the default baseline (``bench suite``
  ...
  preset (general-purpose) and the mobilisation preset (escapes
...
- **Tabu search** demonstrates diversification on synthetic bundles:
  objective traces show convergence parity with SA but at lower runtime.
  The manuscript will highlight how automatic tenure selection removes
  another tuning burden.

These notes accompany the shared ``heuristics_matrix.csv`` so both the
PDF and Sphinx docs can present identical solver guidance.

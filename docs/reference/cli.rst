...
- ``fhops plan rolling`` — execute rolling-horizon solves (stub/SA/MIP hooks) with exports for per-iteration telemetry and locked assignments; see :doc:`../howto/rolling_horizon`. Requires ``master_days`` to fit the scenario horizon and supports solver-specific options (``--sa-iters``, ``--sa-seed``, ``--mip-solver``, ``--mip-time-limit``).

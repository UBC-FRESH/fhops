...
- Added docstrings for all CLI dataset enums (ADV/TN/TR/Spinelli/Loader/Skidder families) plus the `_apply_*` default-merging helpers and key telemetry renderers so the API and Typer help now explain presets and the CLI output helpers; remaining CLI module work is tracked in `notes/sphinx-documentation.md`.

...
- Added docstrings for all CLI dataset enums (ADV/TN/TR/Spinelli/Loader/Skidder families) so the API and Typer help now explain what each preset represents; outstanding work on the `_apply_*` helpers/telemetry renderers is tracked in `notes/sphinx-documentation.md`.

# 2026-09-07 — SoftwareX revision: typesetting pass (list formatting, dash hygiene, table fitting)
- **Final table-regeneration closure:** executed `build_tables.py` against the shipped benchmark/tuning inputs rather than preserving stale manuscript-ready copies. Synchronized med42 solver rows (SA −27271.22/1245.04 s; ILS −33452.28/1791.73 s; Tabu −46063.73/7391.10 s) and the best tuning row (Bayes −35485.30, Δ −8214.08, 5.29 s) through canonical assets, standalone manuscript mirrors, prose, marked PDF, and reviewer response. Regeneration into a separate clean directory now reproduces both canonical CSV tables byte-for-byte.
- **Cover letter and response PDFs prepared for EM:** added `docs/softwarex/tmp/cover-letter.tex` (one-page editor letter) and a five-page point-by-point response built from the corrected `response-letter.md` via a limited Markdown-to-LaTeX converter. Reconciled stale response details before rendering: manuscript number now R1, wrong PRISM authors corrected to Nguyen/Henderson/Wei, changed-file claims aligned with the submitted-base port, and the obsolete deadline removed from the response header. Final upload files copied to the standalone manuscript build directory as `cover-letter.pdf` and `response-to-reviewers.pdf`; both have no missing glyphs, Markdown artifacts, overfull content, or margin violations.
- **EM preview audit fixes:** corrected `build_tables.py` med42 objective sense from `minimize` to `maximize`. The underlying tuner comparison now correctly selects Bayes (−35485.30, 5.29 s), not Random (−39725.88), and reports signed Δ −7214.08 against the manuscript SA baseline (−28271.22); synchronized `assets/data/tables/tuning_leaderboard.{csv,tex}` and the standalone manuscript mirrors. Removed the redundant `Published online` BibTeX note that rendered as `Forest EngineeringPublished online`; the year/DOI remain and the response letter retains the online date. The standalone manuscript package and clean additions-only marked PDF were rebuilt and smoke-tested after these fixes.
- **Response-letter accuracy pass** (after a simulated fresh Reviewer #2 cross-check of letter vs. revised PDF vs. shipped code/assets in `docs/softwarex/tmp/`): corrected the fabricated reference count "9 → 27" to "9 → 15" (15 rendered refs [1]–[15] in the revised PDF; both mentions); rewrote the R1.3 ω-calibration response to match the verified ground truth — reference YAMLs (tiny7/small21/med42/large84) declare prod=1.0/mob=0.5, tiny7/small21 additionally auto-apply (1.0, 0.2, 0.1, 0.05) inside the SA/ILS/Tabu drivers via `AUTO_OBJECTIVE_WEIGHT_OVERRIDES` in `src/fhops/optimization/heuristics/common.py` (NOT in scenario.yaml as previously claimed), med42 uses 1.0/0.5 (not "defaults"), and the synthetic tiers declare no objective_weights so defaults (1.0/1.0/0/0) apply — verified against `docs/softwarex/assets/data/tuning/telemetry/runs.jsonl` config.objective_weight_overrides; fixed section citations (§4.3 "Project maturity" → §4 Impact, and "Section 5 (Impact)" → §4); corrected the thesis citation-location list (Abstract/§1/§3/§4/§5 → §1/§4/§5) and dropped the overclaim that it addresses harvest-system transferability (it is geographic breadth within the same archetype); removed the false synthetic-small runtime claim; listed PRISM as the fifth new reference (was "four"). The manuscript-side ω note and Table 5 prose were corrected in the `fhops-manuscript` repo; the letter now matches that corrected manuscript.
- Removed the fig  `suptitle("Playback robustness: deterministic vs stochastic utilisation")` from `plot_playback_variability.py`: it overlapped the top-anchored legend and duplicated the manuscript caption ("Deterministic vs. stochastic utilization across scenarios."). Regenerated PNG (3570×1253), re-copied into the standalone manuscript repo; top band is now a single centered legend strip (x 1360–2211), title band gone. Rebuilt: Fig. 2 p30, blank 21px band above first ink.
- Figure 2 legend fix propagated to the standalone manuscript repo (`fhops-manuscript`, branch `revision/softx-r1`): the regenerated PNG was re-copied into `manuscript/sections/includes/utilisation_robustness.png` (the earlier 2026-09-07 fix had regenerated the asset here but the manuscript copy still carried the clipped 3600×1200 version). Also hardened `plot_playback_variability.py` with `bbox_extra_artists=[legend]` on both `savefig` calls so older matplotlib versions cannot drop `fig.legend` from `bbox_inches="tight"` again. Rebuilt manuscript PDF: Fig. 2 renders with visible legend (blank 21px band above first ink at 300 dpi), image 110.9–499.4pt on p30, no clipping.
- Fixed Figure 2 legend clipping: `plot_playback_variability.py` anchored the legend via `bbox_to_anchor=(0.5, 1.05)` (above the axes) but saved with a fixed figure box, so the legend's top strokes were cut at the PNG top edge. Both `savefig` calls now pass `bbox_inches="tight"`; regenerated `utilisation_robustness.{png,pdf}` from existing playback CSVs (values unchanged, verified top rows now blank). Cleanup: left a workspace matplotlib install in place (venv-only, not a repo change).
- Fixed Table 3 overflowing the bottom of page 42 (dense `tabularx` taller than the page): typeset at `\footnotesize`; rebuilt PDF shows it tops out at y=704 on a 792pt page. Also scaled Figure 1's PRISMA `tikzpicture` to `\linewidth` via `\resizebox` and dropped its node font from `\footnotesize` to `\scriptsize` (the fixed `text width`s meant font-size alone didn't narrow it — drawing bbox 124–559 now fits inside the ~499 text block).
- Converted the three-contribution list in `introduction.tex` from inline numbered prose (`1. ... 2. ... 3. ...`) to a proper `enumerate` list with full-stop terminations, so the three contributions typeset as a real list.
- Replaced all spaced en-dash list separators (`–`, ` – `) in `software_description.tex` and `illustrative_example.tex` (28 occurrences) with unspaced TeX em dashes (`---`), matching proper scientific typesetting; verified 0 spaced em/en dashes in the rendered PDF (“=, =, =-glyph audit clean).
- Wrapped the two auto-generated performance tables (`solver_performance.tex`, `tuning_leaderboard.tex`) into `\resizebox{\linewidth}{!}{...}` inside `illustrative_example.tex`; the raw 7-/6-column `tabular` inputs overhung the right margin by ~12–36pt on the appendix pages.
- Wrapped the bare `tuning/playback/costing/scaling` token in `illustrative_example.tex` (reproducibility paragraph) in `\path{...}` so it gets break points instead of overflowing the line.
- Rebuilt the manuscript PDF with the Tectonic mirror workflow and re-verified: 45 pages, 0 text/markdown artifacts, 0 spans ≥577pt past the right margin, contribution list typeset as 1./2./3. only on the expected page.
- Command cadence: TeX-only, no Python/lint changes; PDF rebuild used `/tmp/opencode/tex/tectonic -p -c minimal -o build fhops-softx.tex` plus a pymupdf span-geometry/font audit script (fitz API: `pymupdf.open().get_text('dict')`).

# 2026-09-07 — SoftwareX round-1 revision (SOFTX-D-26-00697) on `revision/softx-r1`
- Branch `revision/softx-r1` created from `main`; no commits yet — all round-1 manuscript edits are uncommitted working-tree changes carried onto the branch.
- Updated the Jaffray et al. systematic-review reference to its published form (`jaffray2026systematic`, IJFE 2026, DOI 10.1080/14942119.2026.2662184) across `abstract.tex`, `introduction.tex`, `impact.tex`, `motivation_story.{md,tex}`, and `references.bib`.
- Added a comparative table of forest-planning tools (`tab:comparison`) plus four new references (`blouin2012fpinterface`, `volpe2011fpinterface`, `ronnqvist2003optimization`, `segura2014dss`) addressing the reviewer's "only 9 references / no tool comparison" concern; FPInterface claims scoped to publicly verifiable facts, OptiFor excluded (unverifiable at operational scope).
- Added the objective-weight (ω) calibration and sign-convention note (`sec:objective-weights`) to `software_description.tex` and fixed the Table 3 prose in `illustrative_example.tex` (Synthetic-small objective −75039.79, production 0.00 m³; Med42 maximise-sense ranking; caption sign-convention note; tuning paragraph reframed).
- Disclosed the author self-citation / COI up front in the introduction and made the single-harvest-system validation limitation explicit in `impact.tex`.
- Cited the companion UBC MASc thesis (`jaffray2026thesis`, Jaffray 2026) — its Chapter 4 is the third paper in the logical sequence (review → FHOPS → rolling-horizon evaluation) and tests FHOPS rolling-horizon design across 3 BC TSAs × 3 problem sizes × 4 horizons × 3 re-optimization frequencies (108 deterministic runs) — in `abstract.tex`, `introduction.tex`, `illustrative_example.tex`, `impact.tex`, and `conclusions.tex`.
- Drafted the point-by-point response letter in `docs/softwarex/tmp/response-letter.md` (reference count updated 26 → 27) and recorded details in `notes/softwarex_manuscript_change_log.md`.
- NOT run in this environment: `make pdf` (no TeX engine available); citations/references statically verified (0 undefined refs, 27/27 cite keys in bib).
